...
- The IB_CLOCK_TIMEDIFF() was removed. It was incorrect and never used.

**Performance**

- Removed extraneous mutexes causing contention in the trafficserver plugin.
- The pcre module now uses a single JIT stack and ovector per-transaction instead of allocating/destroying per-execution.
- The pcre module now uses the new fast path JIT API when available. Note that JIT use prior to 8.32 is not recommended due the stack size being limited to the internal 32KB as the pcre_assign_jit_stack() call is not thread safe when storing the "extra" data for JIT read-only as we do. The new fast path API allows for avoiding the pcre_assign_jit_stack() call.
...
- Backported fixes from 0.13 to fix "bandwidth" in txlogs as well as include the "path" in txlogs. See Incompatibilies below.
- Backported pcre module from master with cleanup and performance fixes.

**Development**

- Add missing headers for use with newer boost versions.

**Performance**

- Remove extraneous mutexes causing contention in the trafficserver plugin.

**Build**

- Lots of minor fixes to the build system - escpecially surrounding (over) linking issues.

**Incompatibilities**

- It was important enough to bump the ABI because of a 0.13.0 backport that was required in 0.12.1. Because of this, any external modules will need to be recompiled against 0.12.1.
