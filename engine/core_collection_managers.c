...
#if ENABLE_JSON
/**
 * Handle managed collection: selection for JSON buffers
 *
 * Examines the incoming parameters; if if it looks like a JSON buffer,
 ...
 *   - IB_Exxx Other error
 */
static ib_status_t core_managed_collection_jsonbuf_register_fn(
    const ib_engine_t             *ib,
    const ib_module_t             *module,
    const ib_collection_manager_t *manager,
    ib_mpool_t                    *mp,
    ...
    void                         **pmanager_inst_data)
{
    ...
    rc = ib_json_decode(mp, param, field_list, &error);
    if (rc != IB_OK) {
        if (error != NULL) {
            ib_log_error(ib, "Error decoding JSON buffer: \"%s\"", error);
        }
        return rc;
    }
...
/**
 * Handle managed collection: JSON buffer populate function
 *
 * @param[in] ib Engine
 * @param[in] tx Transaction to populate
 * @param[in] module Collection manager's module object
 ...
 *                collection_data.
 * @param[in] manager_inst_data Manager instance data
 * @param[in] populate_data Callback data
 *
 * @returns
 *   - IB_OK on success or when @a collection_data is length 0.
 *   - The first error returned by a call to ib_field_copy or ib_list_push.
 ...
 *     collection population continues.
 */
static ib_status_t core_managed_collection_jsonbuf_populate_fn(
    const ib_engine_t             *ib,
    const ib_tx_t                 *tx,
    const ib_module_t             *module,
    const ib_collection_manager_t *manager,
    ...
    void                          *populate_data)
{
    ...
    return ib_managed_collection_populate_from_list(tx, field_list, collection);
}
...
#if ENABLE_JSON
    /* Register the JSON buffer InitCollection manager */
    rc = ib_managed_collection_register_manager(
        ib, module, "core JSON buffer", "json:",
        core_managed_collection_jsonbuf_register_fn, NULL,
        NULL, NULL,
        core_managed_collection_jsonbuf_populate_fn, NULL,
        NULL, NULL,
        &manager);
    if (rc != IB_OK) {
        ib_log_alert(ib, "Failed to register core JSON buffer handler: %s",
                     ib_status_to_string(rc));
        return rc;
    }
