                                      ...
                                      ib_field_t **result)
{
    ...
    if (value == NULL) {
        ...
        *result = value;
        ib_rule_log_full(tx, rule, target, NULL, "No transformations");
        IB_FTRACE_RET_STATUS(IB_OK);
    }

    ib_rule_log_full(tx, rule, target, NULL, "Executing %d transformations",
    ...
    IB_LIST_LOOP(target->tfn_list, node) {
        ...
        /* Run it */
        ++n;
        ib_rule_log_full(tx, rule, target, tfn,
                                         ...
                                         ib_num_t *rule_result)
{
    ...
    if ( (value != NULL) && (value->type == IB_FTYPE_LIST) ) {
        ...
        ib_rule_log_full(tx, NULL, target, NULL,
                         "Operator (list %zd) => %d",
    ...
    ib_list_t *target_results)
{
    ...
    rc = execute_field_tfns(ib, tx, rule, target, operand, &tfnvalue);
    if (rc != IB_OK) {
        ib_rule_log_error(tx, rule, target, NULL,
                          "Error executing transformation : %s",
                          ib_status_to_string(rc));
        IB_FTRACE_RET_STATUS(rc);
    }
    ...
    ib_rule_log_full(tx, rule, target, NULL, "Operator result => %d", result);
                                              ...
                                              ib_list_t *target_results)
{
    ...
    if (ib_flags_all(rule->flags, IB_RULE_FLAG_EXTERNAL) == IB_TRUE) {
    ...
    ib_rule_log_full(tx, rule, NULL, NULL, "Executing rule");
    ...
    ib_rule_log_full(tx, rule, NULL, NULL, "Rule operator => %d", *rule_result);
                                  ...
                                  ib_action_inst_t *action)
{
    ...
    ib_rule_log_full(tx, rule, NULL, NULL,
                                      ...
                                      ib_num_t *rule_result)
{
    ...
    *rule_result = 0;
    ...
    if (ib_rule_log_exec_level(ib) != IB_RULE_LOG_EXEC_OFF) {
    ...
    if (ib_rule_log_exec_level(ib) != IB_RULE_LOG_EXEC_OFF) {
        assert(results != NULL);
        ib_rule_log_exec(tx, rule, results_type, results, actions);
    }
    else {
        assert(results == NULL);
    }
                                   ...
                                   void *cbdata)
{
    ...
    if (IB_LIST_ELEMENTS(rules) == 0) {
        ib_rule_log_full(tx, NULL, NULL, NULL,
                                              ...
                                              ib_num_t *result)
{
    ...
    ib_rule_log_full(tx, rule, NULL, NULL, "Operator => %d", *result);
 ...
 */
static void set_rule_enable(ib_bool_t enable,
                                ...
                                ib_list_t *ctx_rule_list)
{
    ...
    const char     *name = enable == IB_TRUE ? "Enable" : "Disable";
    const char     *lcname = enable == IB_TRUE ? "enable" : "disable";
    ...
    case RULE_ENABLE_ALL :
        IB_LIST_LOOP(ctx_rule_list, node) {
            ...
            ib_log_debug2_cfg_ex(ib, ctx->mp,
                                 match->file, match->lineno,
                                 "%sd rule matched \"%s\" by ALL",
                                 name, ctx_rule->rule->meta.id);
        }
        if (matches == 0) {
            ib_log_warning_cfg_ex(ib, ctx->mp, match->file, match->lineno,
                                  "No rules by ALL to %s",
                                  match->enable_str, lcname);
        }
        else {
            ib_log_debug_cfg_ex(ib, ctx->mp, match->file, match->lineno,
                                "%sd %u rules by ALL",
    ...
    case RULE_ENABLE_ID :
         ...
         * IDs are unique */
        ib_log_debug3_cfg_ex(ib, ctx->mp, match->file, match->lineno,
        ...
        ib_log_error_cfg_ex(ib, ctx->mp, match->file, match->lineno,
                                     ...
                                     ib_context_t *ctx)
{
    ...
    IB_LIST_LOOP(ctx->rules->disable_list, node) {
        ...
        /* Apply disable */
        rc = enable_rules(ib, ctx, enable, IB_FALSE, all_rules);
        if (rc != IB_OK) {
    ...
    IB_LIST_LOOP(ctx->rules->enable_list, node) {
        ...
        /* Find rule */
        rc = enable_rules(ib, ctx, enable, IB_TRUE, all_rules);
        if (rc != IB_OK) {
            ib_log_error_cfg_ex(ib, ctx->mp, enable->file, enable->lineno,
