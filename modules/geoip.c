...
)
{
    ...
    if (geoip_id > 0) {
        ...
        if (tmp_str)
        {
            rc = ib_bytestr_dup_nulstr(&tmp_bs, mm, tmp_str);
            if (rc != IB_OK) {
    ...
    else
    {
        ...
        rc = ib_bytestr_alias_nulstr(&tmp_bs, mm, "01");
        if (rc != IB_OK) {
        ...
        rc = ib_bytestr_alias_nulstr(&tmp_bs, mm, "001");
        if (rc != IB_OK) {
...
static ib_status_t geoip_init(ib_engine_t *ib, ib_module_t *m, void *cbdata)
{
    ...
    mod_data = ib_mm_calloc(ib_engine_mm_main_get(ib),
                            sizeof(*mod_data), 1);
    if (mod_data == NULL) {
        return IB_EALLOC;
    }
