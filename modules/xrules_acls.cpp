...
:
    Action("BlockAllow", priority, "", "xrule/acl"),
    m_block(block)
{
    logevent_msg() = (block)? "Block" : "Allow";
}
...
:
    Action("SetFlag_" + field_name, priority, "SetFlag", "xrule/acl"),
...
:
    Action(
        std::string("ScaleThreat_") + unique_id,
        priority,
        "ScaleThreat",
        "xrule/acl"
    ),
...
:
    SetFlag(
        ...
        priority)
{
    logevent_msg() = "SetBlockingMode";
    logevent_tag() = "xrule/acl";
}
...
:
    UnsetFlag(
        ...
        priority)
{
    logevent_msg() = "UnsetBlockingMode";
    logevent_tag() = "xrule/acl";
}
...
)
{
    if (actions.overrides(m_action))
    {
        ...
        const ib_logevent_t *logevent = logevents.back();
        ...
        if (logevent->suppress == IB_LEVENT_SUPPRESS_NONE) {
            ...
            {
                /* Never examine a tag prefixed with "xrule/" as it
                 * should have only been generated by an XRule, and thus
                 * could cause an infinite loop of event generation and
                 * event examination. */
                if (strncmp(event_tag, "xrule/", 6) == 0) {
                    continue;
                }
