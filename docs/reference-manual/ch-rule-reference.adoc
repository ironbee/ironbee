...
Same as the <<_b64_decode>> transformation, but with a web friendly alphabet.
...
This is nearly identical to the <<_match>> operator, but uses the string set instead of a hash as the underlying implementation. For larger sets of words this may be a better choice, but does not offer a case insensitive version, so you would need to do a <<_lowercase>> transformation or similar.
...
This is similar to the <<_strmatch>> operator, but is a longest prefix match instead of a full match.
