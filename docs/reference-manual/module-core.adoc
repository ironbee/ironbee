...
|		Type|Directive
|     Syntax|`AuditEngine On \| Off \| RelevantOnly \| EventsOnly \| AlertsOnly`
...
Setting `AuditEngine` to `EventsOnly`, the default, does not log any transactions in itself. Instead, at least one event is required for a transaction to be recorded. Setting `AuditEngine` to `AlertsOnly` is similar to `EventsOnly`, but there must be one *alert type* event recorded. Setting `AuditEngine` to `On` activates audit logging for *all transactions*, which may cause a large amount of data to be logged.
...
NOTE: As of v0.12.0, `ReleventOnly` is deprecated in favor of `EventsOnly`.
