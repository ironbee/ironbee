...
SetPredicateVars will set +PREDICATE_VALUE+ and +PREDICATE_VALUE_NAME+ to the value and name of +value_provider+ every time before trying to finish the +value_consumer+ node.

*Arguments*: Child one provides a single value whose value and name are cached and assigned to the vars
+PREDICATE_VALUE+ and +PREDICATE_VALUE_NAME+ before the second child is attempted to be finished.
The second child is any node who relies on the name and value of the the first child's result. +
*Result*: The value of the first child. +
*Finished*: When all children are finished. This ensures that the second child is continually given
opportunities to finish using the vars defined by the value result from the first child.
