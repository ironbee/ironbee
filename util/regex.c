...
ib_rx_t *ib_rx_compile(ib_mpool_t *pool, const char *pattern)
{
    ...
    if (!endp) { /* there's no delim  or flags */
        if (ib_regcomp(&ret->rx, pattern, 0) == 0) {
            ib_mpool_cleanup_register(pool, (ib_mpool_cleanup_fn_t)ib_regfree,
                                      &ret->rx);
            return ret;
        }
        else {
    ...
    if (ib_regcomp(&ret->rx, rxstr, ret->flags) == 0) {
        ib_mpool_cleanup_register(pool, (ib_mpool_cleanup_fn_t)ib_regfree,
                                  &ret->rx);
    }
    else {
    ...
    if (!(ret->flags & IB_REG_NOMEM)) {
        ...
        //ret->pmatch = ib_mpool_calloc(pool, ret->nmatch, sizeof(ib_regmatch_t));
    }
    return ret;
}
               ...
               char **newpattern, ib_rxmatch_t *match)
{
    ...
    ib_regmatch_t tmp_pmatch[IB_MAX_REG_MATCH];
    ...
    if (match) {
        ...
        match->pmatch = ib_mpool_calloc(pool, match->nmatch, sizeof(ib_regmatch_t));
    }
    else {
    ...
    if (ib_regexec(&rx->rx, pattern, match->nmatch, match->pmatch, rx->flags)
                     != 0) {
        match->match = NULL;
...
{
    return (match->match != NULL) ? match->nmatch : 0;
}
#endif
